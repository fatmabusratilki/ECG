...
def detect_tpeaks_tends(ecg: np.ndarray, fs: float, r_peaks: np.ndarray, qrs_offsets: np.ndarray
    ...
    t_sig = butter_lowpass(ecg, fs, 8.0, order=3)
    t_peaks = np.full(len(r_peaks), -1, dtype=int); t_ends = np.full(len(r_peaks), -1, dtype=int)
    for i, rp in enumerate(r_peaks):
        start = qrs_offsets[i] if (i < len(qrs_offsets) and qrs_offsets[i] >= 0) else rp + int(0.08*fs)
        start = max(start, rp + int(0.08 * fs)); end = min(len(ecg)-1, rp + int(0.60 * fs))
        if start >= end: continue
        seg = t_sig[start:end+1]; 
        if seg.size < 3: continue
        tpk = start + int(np.argmax(np.abs(seg)))
        base_a = min(len(ecg)-1, start + int(0.02 * fs))
        base_b = max(start, tpk - int(0.02 * fs))
        baseline = float(np.median(t_sig[base_a:base_b])) if base_b > base_a else float(np.median(t_sig[start:tpk])) if tpk>start else float(np.median(t_sig[start:end]))
        t_amp = abs(float(t_sig[tpk] - baseline)); amp_thr = max(0.02 * t_amp, 1e-6)
        slope = np.diff(t_sig, prepend=t_sig[0]); slope_thr = max(0.02 * np.max(np.abs(slope[start:end+1])), 1e-6)
        calm = max(1, int(0.020 * fs)); tend = -1; j = tpk
        while j < end - calm:
            a = j; b = j + calm
            if np.all(np.abs(t_sig[a:b] - baseline) < amp_thr) and np.all(np.abs(slope[a:b]) < slope_thr):
                tend = b; break
            j += 1
        t_peaks[i], t_ends[i] = tpk, (tend if tend >= 0 else -1)
...
def beats_to_dataframe(beats: List[Beat], fs: float):
    ...
    for b in beats:
        rows.append({
            "beat_index": b.index, "t_s": b.time_s, "r_index": b.r_index,
...
def compute_summary(per_beat_df, fs: float) -> Dict[str, Any]:
    ...
    if rr.size:
        ...
        rel = np.abs(d[:-1]) / np.maximum(rr[:-1], 1e-9)
...
def analyze_ecg(ecg: np.ndarray, fs: float, enable_notch: bool = True, mains_hz: float = 60.0
...
def get_args_or_defaults():
    ...
    if args.file is None:
        args.file = r"\\MGBMAD3ISILON1-smb.partners.org\MGH-BTSDATA-ARCHIVE\2019\06\3254689660.hd5"
...
def main():
    ...
    # Run analysis
    print("\nAnalyzing ...")
    per_beat_df, summary = analyze_ecg(ecg, fs, enable_notch=True, mains_hz=args.mains_hz)
