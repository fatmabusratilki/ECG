...
def analyze_csv(csv_path: str, **kwargs) -> Tuple[Dict, pd.DataFrame, Dict]:
    """Load RR from CSV (cleaned_rr.csv format) and run analyze_rr()."""
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"CSV file not found: {csv_path}")
    
    df = pd.read_csv(csv_path)
    
    # Priority: RR_next_ms > RR_prev_ms > rr_ms
    if "RR_next_ms" in df.columns:
        rr_ms = df["RR_next_ms"].dropna().values
        source_col = "RR_next_ms"
    elif "RR_prev_ms" in df.columns:
        rr_ms = df["RR_prev_ms"].dropna().values
        source_col = "RR_prev_ms"
    elif "rr_ms" in df.columns:
        rr_ms = df["rr_ms"].dropna().values
        source_col = "rr_ms"
    else:
        raise ValueError(f"CSV must contain 'RR_next_ms', 'RR_prev_ms', or 'rr_ms'. Found: {list(df.columns)}")

    summary, segments = analyze_rr(rr_ms, **kwargs)
    meta = {"rr_source": f"csv:{csv_path}[{source_col}]", "n_beats": int(len(rr_ms))}
...
def run_with_params(h5_path: str = DEFAULT_H5,
    ...
    # Optional: list datasets (HDF5 only)
    if list_only:
        if csv_path:
            print("List datasets is not supported for CSV input.")
            return
        try:
    ...
    try:
        if csv_path:
            ...
            summary, segments, meta = analyze_csv(csv_path, 
                                                fs_interp=fs_interp, 
            ...
            print(f"Analyzing HDF5: {h5_path}")
            summary, segments, meta = analyze_hdf5(h5_path, dataset_path=dataset,
                                                 fs_interp=fs_interp, psd_method=psd_method, ar_order=ar_order,
                                                 do_segments=do_segments, window_sec=window_sec, step_sec=step_sec)
    except Exception as e:
    ...
    # 2. Export
    out_files = export_results(summary, segments, out_prefix)
    print("\nDone. Generated:")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="HRV Analysis (HDF5 or CSV)")
    parser.add_argument("--file", default=DEFAULT_H5, help="Path to .hd5 file")
    parser.add_argument("--dataset", default=DEFAULT_DATASET, help="HDF5 dataset path (optional)")
    parser.add_argument("--csv", default=None, help="Path to cleaned RR CSV file (overrides HDF5)")
    parser.add_argument("--out", default=DEFAULT_OUT_PREFIX, help="Output filename prefix")
    parser.add_argument("--list", action="store_true", help="List HDF5 datasets and exit")
    parser.add_argument("--json", action="store_true", help="Also emit summary.json")
    
    # Analysis params
    parser.add_argument("--window", type=int, default=300, help="Window size (sec)")
    parser.add_argument("--step", type=int, default=60, help="Window step (sec)")
    parser.add_argument("--no_segments", action="store_true", help="Disable segmented (rolling) metrics")
    
    args = parser.parse_args()

    run_with_params(h5_path=args.file,
